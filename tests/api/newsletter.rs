...
async fn newsletters_are_not_delivered_to_unconfirmed_subscribers() {
    ...
    let newsletter_request_body = serde_json::json!({
        ...
        "content" : {
            ...
            "html": "<p>Newsletter body as html text</p>",
        }
    });
...
#[tokio::test]
async fn newletters_returns_400_for_invalid_data() {
    // Arrange
    let app = spawn_app().await;
    let test_cases = vec![
        (
...
async fn create_unconfirmed_subscriber(app: &TestApp) -> ConfirmationLinks {
    ...
    // We now inspect the requests received by the mock server
    // to retieve the confirmation link and return it
...
async fn create_confirmed_subscriber(app: &TestApp) {
    // We can then resuse the same helper and just add
